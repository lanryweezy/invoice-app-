      ...
      try {
        ...
        try {
          ...
          if (options.from) {
            ...
            invoices = invoices.filter((inv) => 
              // ⚡ Bolt: Use Date.parse instead of new Date().getTime() in loops to avoid object allocation overhead
              Date.parse(inv.createdAt || '') >= fromTime
            );
          }

          if (options.to) {
            const toTime = Date.parse(options.to);
            invoices = invoices.filter((inv) => 
              // ⚡ Bolt: Use Date.parse instead of new Date().getTime() in loops to avoid object allocation overhead
              Date.parse(inv.createdAt || '') <= toTime
            );
          }
          ...
          invoices.sort((a, b) => {
            switch (options.sort) {
              ...
              case 'date':
              default:
                // ⚡ Bolt: Use Date.parse instead of new Date().getTime() in sort callbacks
