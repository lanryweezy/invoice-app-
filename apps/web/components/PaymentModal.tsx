            ...
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="p-6 border-b border-slate-100">
                    <div className="flex justify-between items-center mb-4">
                        <h2 id="payment-modal-title" className="text-xl font-bold text-slate-900">Record Payment</h2>
                    ...
                    <div className="bg-slate-50 rounded-xl p-4 flex justify-between items-center">
                        <div>
                            <p className="text-xs font-bold text-slate-400 uppercase">Invoice #{invoice.invoiceNumber}</p>
                            <p className="text-sm text-slate-600">{invoice.client.name}</p>
                        </div>
                        <div className="text-right">
                            <p className="text-xs text-slate-400">Total Due</p>
                            <p className="text-lg font-black text-slate-900">{invoice.currency} {numberFormatter.format(totalAmount)}</p>
                        </div>
                    </div>
                </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                    {/* Amount */}
                    <div>
                        <label htmlFor="amountPaid" className="block text-sm font-semibold text-slate-700 mb-2">Amount Paid</label>
                        <div className="relative">
                            <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 font-bold">{invoice.currency}</span>
                            <input
                                id="amountPaid"
                                type="number"
                                step="0.01"
                                ...
                                className="w-full pl-16 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-xl font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-3 gap-2">
                            {(showAllMethods ? [
                                ...
                                >
                                    ...
                                    <p className="text-xs font-bold text-slate-900 mt-1">{m.value}</p>
                                </button>
                            ))}
                        </div>
                        {!showAllMethods && (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div>
                            <label htmlFor="paymentDate" className="block text-xs font-bold text-slate-500 mb-1.5">Date</label>
                            <input
                                id="paymentDate"
                                type="date"
                                required
                                ...
                                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all"
                            />
                        </div>
                        <div>
                            <label htmlFor="txnRef" className="block text-xs font-bold text-slate-500 mb-1.5">Reference (optional)</label>
                            <input
                                id="txnRef"
                                type="text"
                                ...
                                className="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm text-slate-900 placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 transition-all"
                            />
                        </div>
                    </div>
                    ...
                    <div className="pt-2 flex gap-3">
                        <button type="button" onClick={onClose} className="flex-1 px-4 py-3 text-slate-700 font-bold bg-slate-100 hover:bg-slate-200 rounded-xl transition-colors text-sm">
                            Cancel
                        </button>
                        <button
                            type="submit"
                            disabled={loading || amountPaid <= 0}
                            className="flex-1 px-4 py-3 text-white font-bold bg-teal-600 hover:bg-teal-700 disabled:bg-slate-300 disabled:cursor-not-allowed rounded-xl shadow-lg shadow-teal-600/20 transition-all text-sm flex items-center justify-center gap-2"
                        >
