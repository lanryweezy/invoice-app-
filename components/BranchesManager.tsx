        ...
        <div>
          <form onSubmit={handleAdd} className="space-y-4 bg-slate-50 p-5 rounded-xl border border-slate-200">
             ...
             <div>
                ...
                <input value={newBranch.name} onChange={e => setNewBranch({...newBranch, name: e.target.value})} className="w-full p-2 border border-slate-300 rounded focus:border-teal-500 outline-none" placeholder="e.g. Kano Branch" required />
             </div>
             <div>
                <label className="block text-xs font-bold text-slate-500 mb-1">Address</label>
                <input value={newBranch.address} onChange={e => setNewBranch({...newBranch, address: e.target.value})} className="w-full p-2 border border-slate-300 rounded focus:border-teal-500 outline-none" placeholder="e.g. 10 Zoo Road" required />
             </div>
           ...
           <div className="space-y-3">
             {branches.map(branch => (
               <div key={branch.id} className="p-4 bg-white border border-slate-200 rounded-lg shadow-sm flex justify-between items-center">
                 ...
                 <button onClick={() => setBranches(branches.filter(b => b.id !== branch.id))} aria-label="Delete branch" className="text-slate-400 hover:text-red-500">
