...
## 2025-05-14 - Unmonitored External Data Fallbacks
**Learning:** Found an observability blind spot in `apps/web/utils/exchangeRates.ts` where the `getExchangeRates` function uses native fetch. When this fetch fails or times out, it gracefully degraded to a set of hardcoded `FALLBACK_RATES`, catching the error with just a generic `console.warn`. This is a dangerous silent failure mode where the business could be running on severely outdated exchange rates in production, and no one would know unless they checked user browser consoles.
**Action:** When implementing robust offline/fallback behaviors for external data dependencies, explicitly emit telemetry (e.g., `exchange_rates_fetch_failed`) in the catch block to ensure the degradation state is visible to developers and the business.
