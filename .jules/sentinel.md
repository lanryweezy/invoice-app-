...
## 2024-05-24 - CLI Token Leakage
**Vulnerability:** The CLI `config get` command exposed sensitive user authentication tokens (`idToken` and `refreshToken`) in plaintext to the console output.
**Learning:** Even when printing configuration objects for debugging or display purposes, sensitive authentication fields must be explicitly filtered or masked. Standard filtering may overlook root-level token properties if only nested secrets (like `smtp.pass`) are targeted.
**Prevention:** Always actively filter or mask authentication tokens (e.g., `idToken`, `refreshToken`) before serializing and outputting configuration objects to the console.
