...
## 2024-07-25 - Missing cursor-pointer on labels
**Learning:** Found that label elements connected via `htmlFor` to input fields in `BranchesManager.tsx`, `AuthModal.tsx`, and `PaymentModal.tsx` lacked a `cursor-pointer` class, which decreases visual affordance for users indicating they can click the text to focus the input.
**Action:** Always add `cursor-pointer` to `htmlFor` labels in React/Tailwind forms to make the clickable hit area visually obvious.
