...
const App: React.FC = () => {
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 text-slate-800 font-sans overflow-hidden selection:bg-teal-100 selection:text-teal-900">
      <Helmet>
        <title>{invoice.invoiceNumber ? `Invoice #${invoice.invoiceNumber} | Naija Invoice Generator` : 'Free Invoice Generator for Nigeria | Naija Invoice'}</title>
        <meta name="description" content={invoice.user.name ? `Invoice generated by ${invoice.user.name} for ${invoice.client.name || 'a client'} using Naija Invoice Generator.` : "Create professional invoices in seconds. The #1 free invoice generator tailored for Nigerian freelancers and businesses."} />
        <link rel="canonical" href="https://naijainvoice.ng/" />

        {/* Dynamic Open Graph */}
        <meta property="og:title" content={invoice.invoiceNumber ? `Invoice #${invoice.invoiceNumber} | Naija Invoice` : 'Free Invoice Generator for Nigeria'} />
        <meta property="og:description" content={invoice.user.name ? `Professional invoice created by ${invoice.user.name} via Naija Invoice.` : 'Create and share professional invoices for free.'} />

        {/* Dynamic Twitter */}
        <meta name="twitter:title" content={invoice.invoiceNumber ? `Invoice #${invoice.invoiceNumber} | Naija Invoice` : 'Free Invoice Generator for Nigeria'} />
        <meta name="twitter:description" content={invoice.user.name ? `Check out this invoice created by ${invoice.user.name} using #NaijaInvoice.` : 'The best way to generate invoices in Nigeria.'} />
      </Helmet>
